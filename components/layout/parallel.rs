...
use incremental::{RestyleDamage, Repaint};
...
fn recalc_style_for_node(mut unsafe_layout_node: UnsafeLayoutNode,
    ...
    debug!("[{}] End style recalc for {:x}. damage={}",
           ...
           ThreadSafeLayoutNode::new(&node).restyle_damage());
    ...
    if dirty_child_count != 0 {
        ...
        match &mut *layout_data_ref {
            &Some(ref mut layout_data) => {
                let data = &mut layout_data.data;
                data.parallel.children_count.store(dirty_child_count as int, Relaxed);
            }
...
fn construct_flows<'a>(unsafe_layout_node: &mut UnsafeLayoutNode,
    ...
    loop {
        ...
        let parent_node = node.layout_parent_node(layout_context.shared);

        {
            let tsln = ThreadSafeLayoutNode::new(&node);

            if node.must_force_reflow() {
                tsln.set_restyle_damage(RestyleDamage::all());
            }
            ...
            || node.has_dirty_descendants() {
                ...
                debug!("[{}] skipping flow construction for {:x}", tid(), unsafe_layout_node.val0());
            }

            // Reset the layout damage in the node. The layout damage has been
            // propagated to the flow by the flow constructor.
            tsln.set_restyle_damage(RestyleDamage::empty());
        }

        unsafe {
            node.set_is_dirty(false);
            node.set_must_force_reflow(false);
            node.set_has_dirty_children(false);
...
fn build_display_list(mut unsafe_flow: UnsafeFlow,
    ...
    loop {
        unsafe {
            ...
            if needs_repaint {
                debug!("Building display list for {}", flow::base(flow.get()).debug_id());

                // Build display lists.
                flow.get_mut().build_display_list(&layout_context);

            } else {
                debug!("Skipping build of display list for {}", flow::base(flow.get()).debug_id());
            }
